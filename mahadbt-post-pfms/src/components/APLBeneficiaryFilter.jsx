...
import { fetchBeneficiaryList, fetchBillList } from '../api/apl-beneficiary-list';
...
const getFinancialYearPicklistCode = (scheme = {}) => {
  ...
  return isPostMatricScheme ? 'DBT-ACADEMIC-YEAR' : 'DBT-FINANCIAL-YEAR-APL';
};

function APLBeneficiaryFilter({ roles, setSearchResults, setSearchData, setScheme, isPensionDDO = false, isAssistanceDDO = false }) {
...
const [lekLadkiInstallments, setLekLadkiInstallments] = useState([]);
  ...
  const isAPLScheme = schemeCode.startsWith('FACS-');
  ...
  const onSubmit = async (data) => {
    ...
    const results = await fetchBillList(data, setScheme, isPensionDDO || isAssistanceDDO);
        ...
        <div className="card-body">
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="row g-3">
              ...
              <div className="col-md-4 mb-3">
                <label className="form-label">
                  {isPensionScheme || isAPLScheme ? 'Month' : isOneTimeScheme ? 'Benefit Type' : isLekLadki ? 'Installment' : 'Installment'}
                </label>
...
export default APLBeneficiaryFilter
