...
export const apiService = {
  ...
  // Update Bill data status (bulk update to SIGNED_BY_DDO)
  updateBilltatus: async (payload) => {
    const response = await api.post("/apl-wip/bill-update", payload);
    return response.data;
  },


  // Get WIP beneficiaries with APPROVED status (for DFSO)
  getBeneficiariesForDisbursement: async (params) => {
    try {
      console.log("Fetching WIP data with status SCRUTINY_PENDING");
  ...
  // Get WIP beneficiaries with APPROVED status (for DFSO)
  getBillDetails: async (params) => {
    try {
      console.log("Fetching WIP data with status SIGNED_BY_DDO");
      ...
      const response = await api.get("/apl-bill/bill-details", {
        params: {
          page: 1,
          limit: 1000,
