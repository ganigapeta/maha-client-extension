...
const BeneficiaryDetails = ({searchResults,apiRes,searchData,setCheckBalance,setApiRes,checkBalance,setSearchResults,setBackupSearchData,backupSearchData}) => {
  const[show,setShow]=useState(false)
 const handleGetCheckBalance = async () => {
  try {
    
    const picklistRes = await getPicklistDefinitionByExternalReferenceCode(
      "DBT-ACADEMIC-YEAR"
    );
    ...
    const fy =
      picklistData.find(
        item => item.key == searchData.financialYear
      )?.name || "";
  ...
  return (
          ...
          <tbody>
          {
              searchResults.length > 0 && (
                <tr>
                  ...
                  <td>
                    <button className="btn btn-sm btn-primary" onClick={handleGetCheckBalance}>Check Balance</button>
                  </td>
                </tr>
              )
             }
    ...
    {
     show &&  <AllocateBeneficiaries checkBalance={checkBalance.data} apiRes={apiRes}  searchResults={searchResults} setSearchResults={setSearchResults} setBackupSearchData={setBackupSearchData} backupSearchData={backupSearchData} searchData={searchData}/>
    }
    </>
  );
};
